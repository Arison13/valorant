  ...
  return (
    <header className='navbar'> 
      {isMobile ? <MdOutlineClose onClick={()=>{setIsMobile(false)}} className='mobile-menu-btn' /> :
       <FaBars onClick={()=> {setIsMobile(true)}} className='mobile-menu-btn'/>}
      ...
      <ul className={`mobile-navbar ${ isMobile ?'nav-open' :'nav-close'}`}> 
          ...
          <li onClick={()=>{setIsMobile(false)}}> <Link to='/agents'>Agents</Link> </li>
          <li onClick={()=>{setIsMobile(false)}}> <Link to='/maps'>Maps</Link> </li>
          <li onClick={()=>{setIsMobile(false)}}> <Link to='/1'>Look Up Match</Link> </li> 
      </ul>
    </header>
  )
}
